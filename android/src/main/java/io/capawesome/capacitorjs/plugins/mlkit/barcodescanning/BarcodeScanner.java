...
public class BarcodeScanner implements ImageAnalysis.Analyzer {
    ...
    public void stopScan() {
        ...
        VoteScanFailureLMT = 0;
        VoteScanSuccessLMT = 0;
    }
            ...
            .addOnSuccessListener(
              barcodes -> {
                ...
                for (Barcode barcode : barcodes) {
                  ...
                  if (votes >= VoteScanSuccessLMT) {
                    handleScannedBarcode(barcode, imageSize);
                    stopScan();
                    break;
                  }

                  if (barcodeScanCounter >= VoteScanFailureLMT) {
    ...
    private void handleScannedBarcode(Barcode barcode, Point imageSize) {
        plugin.notifyBarcodeScannedListener(barcode, imageSize, barcodeRawValueVotes);
    }
    ...
    private GmsBarcodeScannerOptions buildGmsBarcodeScannerOptions(ScanSettings scanSettings) {
        int[] formats = scanSettings.formats.length == 0 ? new int[] { Barcode.FORMAT_ALL_FORMATS } : scanSettings.formats;
        GmsBarcodeScannerOptions options = new GmsBarcodeScannerOptions.Builder().setBarcodeFormats(formats[0], formats).enableAutoZoom().build();
        return options;
    }
  ...
  public void setScanLimits(SetScanLimitOptions options) {
